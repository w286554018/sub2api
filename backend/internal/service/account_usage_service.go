...
type UsageCache struct {
	...
	openAIProbeCache  sync.Map           // accountID -> next permitted quota refresh time
...
func (s *AccountUsageService) getOpenAIUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	...
	if (force || shouldRefreshOpenAICodexSnapshot(account, usage, now)) && s.shouldProbeOpenAICodexSnapshot(account.ID, now, force) {
		if account.IsShadow() {
			...
			// Usage refresh is read-only. Keep the previous snapshot and local
			// usage-log statistics when quota is unavailable; never infer.
			if quotaUsage, err := s.openAIQuotaService.QueryUsage(ctx, account.ID); err == nil {
				if updates := buildCodexPrimaryWindowExtraUpdates(quotaUsage, now); len(updates) > 0 {
					mergeAccountExtra(account, updates)
					s.persistOpenAICodexProbeSnapshot(account.ID, updates)
					if usage.UpdatedAt == nil {
						usage.UpdatedAt = &now
...
func isOpenAICodexSnapshotStale(account *Account, now time.Time) bool {
	...
	// Ordinary and Spark OAuth snapshots use the quota endpoint, independently
	// of inference transport. The admission cache separately limits refreshes.
