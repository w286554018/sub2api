...
func (s *DashboardService) GetGroupDetailStats(ctx context.Context, id int64, from, to *time.Time) (*GroupDetailStats, error) {
	if id <= 0 || (from != nil && to != nil && !from.Before(*to)) {
		return nil, infraerrors.BadRequest("GROUP_STATS_INVALID_RANGE", "group stats range is invalid")
	}
	if s == nil {
		return nil, infraerrors.ServiceUnavailable("GROUP_STATS_UNAVAILABLE", "group stats service is unavailable")
	}
