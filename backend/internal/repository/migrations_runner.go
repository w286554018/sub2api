...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	// Some existing deployments applied migration 145 from a vendor build that
	// widened the same platform CHECK constraint beyond Kiro (the live database
	...
	// 157 的另一 checksum 也已在既有生产库中观察到。兼容范围严格限制为这些
	// 已知 checksum，两版约束的差异由 239_fork_platform_constraints_superset 统一收敛。
