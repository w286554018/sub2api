      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAI" @click="$emit('runtime', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="terminal" size="sm" class="text-cyan-500" />
              {{ t('admin.accounts.runtime.view') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'runtime', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
